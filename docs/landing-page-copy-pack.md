...
英文主軸需要比一般工具文案更有文學感。aDict 可以像頁邊的註腳、讀者手邊的索引、或舞台邊等待提示的 prompter：它不搶走文本，只在某個詞讓句子停住時出現。

首屏可以使用 `page`、`line`、`margin`、`scene`、`spell` 這類語彙，讓產品靠近閱讀、文學與戲劇；但不要寫成仿古英文，不要使用 `thou`、`thee`、`wherefore` 之類會變成莎士比亞 cosplay 的詞。
...
- 文學感來自「閱讀現場」與「頁邊陪伴」，不是華麗辭藻。
- 莎士比亞感可以借用舞台、台詞、幕間、旁白、提示者的意象，但不直接模仿古英語。
...
```text
aDict 3.0 · Dictionaries in the margin
```
...
```text
Keep your dictionaries in the margin: Youdao, local MDict and StarDict files, favorites, and iCloud documents wait quietly beside the line, ready when a word asks to be understood.
```
...
```text
aDict 像留在頁邊的詞典：有道、本地 MDict 與 StarDict 詞庫、收藏與 iCloud 文件都在手邊，讓需要的意思出現，而不把你帶離文字現場。
```
...
```text
Youdao · MDict · StarDict · iCloud Documents
```
...
```text
When a word interrupts the passage, aDict keeps online lookup, personal dictionary files, and saved words close enough to consult without leaving the page.
```

繁中：

```text
為那些讓句子停住的詞，準備一位安靜的提示者。
```

```text
當一個詞打斷段落，aDict 把線上查詞、個人詞庫與收藏留在頁邊，讓查詢不必變成另一趟搜尋旅程。
```
...
```text
For readers who bring their own references
```
...
```text
aDict brings Youdao, local MDX and StarDict dictionaries, favorites, and iCloud documents into one Apple-platform reading companion.
```
...
```text
aDict 把有道、本地 MDX 與 StarDict 詞庫、收藏與 iCloud 文件放進同一個 Apple 平台閱讀伴侶裡。
```

### 方案 D：短句版本

適合首屏字級很大、畫面需要更像一句文學標題時使用。
...
```text
aDict gives each difficult word a quiet place in the margin: online lookup, local MDX and StarDict files, favorites, and iCloud documents, all without breaking the reading scene.
```
...
```text
aDict 在頁邊為難懂的詞留下一個安靜位置：線上查詞、本地 MDX 與 StarDict 詞庫、收藏與 iCloud 文件，都不打斷閱讀現場。
```
